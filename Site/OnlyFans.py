...
try:
    _onlysnap_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "OnlySnap.py")
...
# Global stop flag for module-level download functions
_global_stop = False
...
class DownloadManager:
    ...
    def run_mass_download(self, username, progress_callback):
        ...
        try:
            ...
            assure_dir("../Profiles/OnlyFans/" + PROFILE)
            assure_dir("../Profiles/OnlyFans/" + PROFILE + "/Public")
            ...
            with open(f"../Profiles/OnlyFans/{PROFILE}/Dump.json", 'w', encoding='utf-8') as f:
            ...
            if not disable_download_txt:
                assure_dir(f"../Profiles/OnlyFans/{PROFILE}/Media")
            ...
            if download_labels_option and not self.stop_requested:
                ...
                if labels_list:
                    for label in labels_list:
                        ...
                        base_label_path = f"../Profiles/OnlyFans/{PROFILE}/Media/Labels/{lname}/"
            ...
            with ThreadPoolExecutor(max_workers=NUM_THREADS) as executor:
                ...
                # A. CHATS -- Added DRM
                if chats_list:
                    cp = f"../Profiles/OnlyFans/{PROFILE}/Media/Chat/Photos/"
                    cv = f"../Profiles/OnlyFans/{PROFILE}/Media/Chat/Videos/"
                ...
                if stories_list:
                    for story in stories_list:
                        for media in story.get("media", []):
                            ...
                            if src:
                                futures.append(executor.submit(download_media, media, False, path=f"../Profiles/OnlyFans/{PROFILE}/Media/Stories/", source_url=src))
                ...
                for post, is_arch, is_str in final_download_list:
                    ...
                    if download_tagged and contains_tags:
                        base_tag = f"../Profiles/OnlyFans/{PROFILE}/Media/Tag-Post"
                    ...
                    if not disable_download_txt and text:
                        ...
                        if is_in_label:
                            txt_dir = label_map[pid]['base']
                             ...
                             txt_dir = f"../Profiles/OnlyFans/{PROFILE}/Media/Posts/{post_date_str}"
...
def create_auth():
    ...
    example_path = auth_file_path + ".example"

    if not os.path.exists(auth_file_path):
        if os.path.exists(example_path):
            shutil.copyfile(example_path, auth_file_path)
            
    try:
        with open(auth_file_path) as f:
            ljson = json.load(f)
    except Exception:
        raise Exception("Missing Auth.json (Please configure it in Site/Configs/OnlyFans)")
...
def load_config():
    ...
    try:
        ...
        raise Exception("Missing Config.json (Please configure it in Site/Configs/OnlyFans)")
...
def download_public_files():
    ...
    for public_file in public_files:
        ...
        full_path = "../Profiles/OnlyFans/" + PROFILE + "/Public" + path
...
def get_year_folder(timestamp, media_type):
    ...
    base_path = "../Profiles/OnlyFans/" + PROFILE + "/Media"
...
def download_media(media, is_archived, path=None, timestamp=None, is_stream=False, source_url=None, post_id=None, specific_cookies=None, is_chat=False):
    ...
    if path is None:
        if is_stream:
            final_path = f"../Profiles/OnlyFans/{PROFILE}/Media/Streams/{file_base_name}{ext}"
        elif is_archived:
            sub_folder = "Photos" if media["type"] == "photo" else "Videos"
            final_path = f"../Profiles/OnlyFans/{PROFILE}/Media/Archived/{sub_folder}/{file_base_name}{ext}"
        else:
            folder_name = get_year_folder(timestamp, "photo" if media["type"] == "photo" else "video")
            type_f = "!Photos" if media["type"] == "photo" else "!Videos"
            final_path = f"../Profiles/OnlyFans/{PROFILE}/Media/{type_f}/{folder_name}/{file_base_name}{ext}"
    else:
        if path.startswith("/"):
            final_path = f"../Profiles/OnlyFans/{PROFILE}{path}{file_base_name}{ext}"
        else:
    ...
    if path is None and not is_stream and not is_archived and type_f:
        ...
        base_media_path = f"../Profiles/OnlyFans/{PROFILE}/Media/{type_f}"
            ...
            for media in post["media"]:
                ...
                if source_url:
                    ...
                    if download_tagged_posts and contains_tags:
                        base_path = f"../Profiles/OnlyFans/{PROFILE}/Media/Tag-Post"
                        path = f"/{base_path}/" if merge_tagged_media else f"/{base_path}/{media['type']}s/"
                    elif save_text_with_media:
                        post_dir = f"../Profiles/OnlyFans/{PROFILE}/Media/Posts/{post_date_str}"
                        assure_dir(post_dir)
...
def download_highlights(highlights, file_callback=None):
    ...
    base_path = f"../Profiles/OnlyFans/{PROFILE}/Media/Highlights/"
...
def download_chats(chats): # Fixed DRM chats
    ...
    chat_path = "../Profiles/OnlyFans/" + PROFILE + "/Media/Chat"
...
def download_stories(stories):
    ...
    assure_dir("../Profiles/OnlyFans/" + PROFILE + "/Media/Stories")
    ...
    with ThreadPoolExecutor(max_workers=NUM_THREADS) as executor:
        ...
        for story in stories:
            ...
            for media_item in media_items:
                ...
                if source_url:
                    futures.append(executor.submit(download_media, media_item, False, path="../Profiles/OnlyFans/" + PROFILE + "/Media/Stories/", source_url=source_url))
                else:
...
if __name__ == "__main__":
    try:
        ...
        if cookies_expired:
            ...
            try:
                import importlib
                cookie_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "cookie-onlyfans.py")
                spec = importlib.util.spec_from_file_location("cookie_of", cookie_path)
                cookie_mod = importlib.util.module_from_spec(spec)
                spec.loader.exec_module(cookie_mod)
                result = cookie_mod.auto_scrape()
                if result:
                    cookies = cookie_mod.parse_cookies(result.get("cookie", ""))
                    cookie_mod.save_auth(result, cookies)
                    print(f"{Fore.GREEN}[*] Cookies refreshed! Reloading...{Style.RESET_ALL}")
                    API_HEADER = create_auth()
                else:
                    print(f"{Fore.RED}[!] Cookie scrape failed. Please update Auth.json manually.{Style.RESET_ALL}")
            except Exception as scrape_err:
                print(f"{Fore.RED}[!] Auto-refresh failed: {scrape_err}{Style.RESET_ALL}")
